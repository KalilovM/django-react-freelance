...
class ExecutorUpdateView(generics.UpdateAPIView):
	"""
	Update one of the executors data
	"""
	queryset = Executor.objects.all()
...
class CreateExecutorView(generics.CreateAPIView):
	...
	queryset = Executor.objects.all()
	serializer_class = CreateExecutorSerializer
	"permisson_classes is to restrict access to unauthorized users"
	permission_classes = (IsExecutor,)

class ExecutorListView(generics.ListAPIView):
	"""
	Get list of the all executors
	"""
	queryset = Executor.objects.all()
	serializer_class = ExecutorSerializer
...
class ServiceListView(generics.ListAPIView):
	...
	def get_queryset(self):
		...
		if price:
			queryset = filter(price__lte=price)
		
		if executor:
			queryset = filter(executor__id=executor)
...
class OrderListView(generics.ListAPIView):
	...
	def get_queryset(self):
		...
		if customer:
			queryset = filter(customer__id=customer)
...
class CreateOrderingView(generics.CreateAPIView):
	queryset = Ordering.objects.all()
	serializer_class = CreateOrderingSerializer
	permission_classes = [permissions.IsAuthenticatedOrReadOnly]
...
class MessageListView(generics.ListAPIView):
	...
	def get_queryset(self):
		...
		params = self.request.qyery_params
		...
		if from_date:
			queryset = filter(msg_date__gte = from_date)

		if to_date:
			queryset = filter(msg_date__lte = to_date)
...
class CreateAuthoringView(generics.CreateAPIView):
	queryset = Authoring.objects.all()
	serializer_class = CreateAuthoringSerializer
	permission_classes = [permissions.IsAuthenticatedOrReadOnly]
